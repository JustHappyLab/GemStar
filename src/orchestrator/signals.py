...
def build_signals(
    ...
    return _build_walk_forward_lstm_signals(features, trade_dates, timer_cfg)


def _build_walk_forward_lstm_signals(
    ...
    del dates_arr
    if len(X) < _MIN_LSTM_TRAIN_SAMPLES:
        return pd.DataFrame({"trade_date": trade_dates, "position": [0.0] * len(trade_dates)})

    label_end_dates = _label_end_dates(features, timer_cfg.seq_len, timer_cfg.horizon)
    raw_signals: list[pd.DataFrame] = []

    for retrain_date, segment_dates in _retrain_segments(
        ordered_trade_dates, timer_cfg.retrain_months,
    ):
        train_mask = label_end_dates < retrain_date
        visible_count = int(train_mask.sum())
        if visible_count < _MIN_LSTM_TRAIN_SAMPLES:
            continue
...
def _label_end_dates(features_df: pd.DataFrame, seq_len: int, horizon: int) -> np.ndarray:
...
def _training_mask_for_cutoff(
    ...
    return _label_end_dates(features_df, seq_len, horizon) < str(cutoff_date)


def _retrain_segments(
    trade_dates: list[str],
    retrain_months: int,
